...
def submitSurvey():
    username = ''
    email = ''
    giphy_api = 'http://api.giphy.com/v1/gifs/random?api_key=dc6zaTOxFJmzC&tag=mean+girls'
    response = requests.get(giphy_api)
    response_json = json.loads(response.text)
    gif_url = response_json['data']['fixed_width_downsampled_url']
    print(gif_url)
